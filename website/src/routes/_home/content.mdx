...
import { BenchPlot } from "@/features/benchmark/components/plot";
...
<Suspense fallback={<Spinner />}>
  <ClientOnly>
    ...
    <p className="typo-caption">For graphs on this page, the best result for each library is shown.</p>
...
<div className="typo-caption">
  Libraries with an asterisk (\*) throw an error when parsing invalid data (and
  have no non-throwing equivalent), so the benchmark includes a try/catch.
</div>
...
Parsing invalid data (aborting early if possible):
