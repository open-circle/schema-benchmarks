...
export interface ToJsonTableProps {
...
const getDefaultCompareId = (results: Array<SchemaToJsonResult>) =>
  (results.find((result) => result.mean) ?? results[0])?.id;

function useComparison(results: Array<SchemaToJsonResult>) {
...
export function ToJsonTable({ results, meanScaler, ...sortState }: ToJsonTableProps) {
