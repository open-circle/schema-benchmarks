...
import { DateDisplay } from "#src/shared/components/date";
import { BaseBenchPlot } from "#src/routes/_benchmarks/_runtime/-components/plot";
...
import benchResults from "./assets/welcome/bench.json";
import updatedBenchResults from "./assets/welcome/bench-update.json";
import downloadResults from "./assets/welcome/download.json";
import updatedDownloadResults from "./assets/welcome/download-update.json";
...
<Admonition type="warning" title="Results">
  The results on this page are correct at time of writing, but will not be kept up to date. For the
  latest results, visit the <Link to="/">homepage</Link> and dedicated benchmark pages.
</Admonition>
...
<Admonition type="info">
  Some libraries have multiple benchmarks for the same thing. Where that's the case, every variant
  is plotted in the same row - hover over a point for details about that specific benchmark.
</Admonition>
...
In order to measure these, we create <Link to="/repo/raw/$" params={{ _splat: "schemas/libraries/valibot/download/index.ts" }} search={{ formatted: true }} target="\_blank" rel="noreferrer noopener">example usage files</Link> for each library, and measure the size of the compiled output (both <Link to="/repo/raw/$" params={{ _splat: "schemas/libraries/valibot/download_compiled/index/minified.js" }} target="\_blank" rel="noreferrer noopener">minified</Link> and <Link to="/repo/raw/$" params={{ _splat: "schemas/libraries/valibot/download_compiled/index/unminified.js" }} target="\_blank" rel="noreferrer noopener">unminified</Link>).

Here are the results for minified and gzipped bundle size:

<BaseDownloadPlot data={downloadResults.minified} />

Two clear leaders emerge:
...
On the opposite end, [`zod`](https://zod.dev)'s "classic" API clocks in with the largest size of 58KB (_see below for an update_), with [`joi`](https://joi.dev) not far behind at 53KB, and [`effect/Schema`](https://effect.website/docs/schema/introduction/) at 50KB (though notably, [`effect`'s beta](https://effect.website/blog/releases/effect/40-beta/) has managed to reduce this significantly, to 19KB).

#### Update (<DateDisplay date="2025-02-22Z" />)

After a closer review, we noticed a discrepancy between our example file for `zod` and its [docs](https://zod.dev), namely that our benchmark was using a _default_ import, and the docs show a _namespace_ import.
...
With this simple change, `zod`'s bundle size plummets to a much more reasonable 16KB.

<BaseDownloadPlot data={updatedDownloadResults.minified} />
...
For each library, we create a <Link to="/repo/raw/$" params={{ _splat: "schemas/libraries/valibot/index.ts" }} search={{ formatted: true }} target="\_blank" rel="noreferrer noopener">`getSchema`</Link> function that returns a schema (with the same validations as the other libraries, to keep the comparison fair). We then benchmark how long it takes to call this function.

Here are the results:

<BaseBenchPlot data={benchResults.initialization} />
...
<BaseBenchPlot data={benchResults.validation.valid} />

Validating invalid data:

<BaseBenchPlot data={benchResults.validation.invalid} />
...
Benchmarks of methods that throw an error (instead of returning one) have to be wrapped in a `try`/`catch` for the benchmark to work, which may have an unknown performance impact.

Benchmarks that abort early (i.e. return the first error found) are relevant when parsing invalid data, as it will (usually) be faster than libraries that return all errors.

Hover over a point in the graph below for these details about that specific benchmark.

</Admonition>

Parsing valid data:

<BaseBenchPlot data={benchResults.parsing.valid} />

Parsing invalid data:

<BaseBenchPlot data={benchResults.parsing.invalid} />
...
<BaseBenchPlot data={benchResults.parsing.valid.filter((d) => d.errorType === "allErrors")} />

Consistently slowest in these tests appears to be [`typebox`](https://github.com/sinclairzx81/typebox) _(see below for an update)_, followed by [`vinejs`](https://vinejs.dev), then [`yup`](https://github.com/jquense/yup) - though
two of these involve a `try`/`catch` wrapper, which may be slowing them down.

#### Update (<DateDisplay date="2025-02-20Z" />)

`typebox`'s slow performance was due to a behavioural change introduced in 1.0, where the schema would actively try to coerce an invalid value into a valid one.

In version 1.1, this was [changed](https://github.com/sinclairzx81/typebox/pull/1532) to be an opt-in feature.
...
Having updated our dependencies (and accepting a [contribution](https://github.com/open-circle/schema-benchmarks/pull/54) of `io-ts` benchmarks while we were at it), `typebox` now performs much better:

<BaseBenchPlot
  data={updatedBenchResults.parsing.valid.filter((d) => d.errorType === "allErrors")}
/>

We'd also like to thank `typebox`'s creator, [`@sinclairzx81`](https://github.com/sinclairzx81), for their assistance [correcting our benchmark](https://github.com/open-circle/schema-benchmarks/issues/55)!
...
Think you can help us out? Great! Check out our <Link to="/contributing">contributing guide</Link>.
