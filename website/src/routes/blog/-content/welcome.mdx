...
Fabian, in his role as the creator of [`valibot`](https://valibot.dev), was well known for his passion for comparing schema validation libraries. However, this was largely confined to his social media posts, and Valibot's website.
...
In early November, Fabian approached me with his idea for a dedicated schema benchmarking project, one that measures all of the different aspects of what can make a schema fast/slow. I was immediately interested, and began work (including creating the logo) the next day.
...
- [Validation](#validation) - How long it takes to check if a
  value matches a schema.
- [Parsing](#parsing) - How long it takes to check if a value
  matches a schema, and return a new value.
...
<p className="typo-caption">Graphs show best result per library</p>
...
We test validation against both valid and invalid data. As validation methods are expected to return a boolean, it's not tracked whether they abort early or not (as it is assumed they would if possible).

Not every library supports validation - for example, [`zod`](https://zod.dev) only supports parsing. In these cases, we categorise them accordingly.
...
We test parsing against both valid and invalid data. We also track whether the library returns the first issue found, or all of them (or both if configurable). This is noted in the graph below with a dagger (†).
...
Consistently slowest in these tests appears to be [`typebox`](https://github.com/sinclairzx81/typebox), followed by [`yup`](https://github.com/jquense/yup) - though both of these involve a `try`/`catch` wrapper, which may be slowing them down.

## Contributing

This project is far from over, and we'd love to hear your ideas!

Check out our <a target="_blank" rel="noreferrer noopener" href="https://github.com/open-circle/schema-benchmarks/blob/main/CONTRIBUTING.md">contributing guide</a> for more information.
