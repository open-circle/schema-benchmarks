...
import { BaseBenchPlot } from "#/routes/_benchmarks/_runtime/-components/plot";
import { BaseDownloadPlot } from "#/routes/_benchmarks/download/-components/plot";
...
Benchmarks of methods that throw an error (instead of returning one) have to be wrapped in a `try`/`catch` for the benchmark to work. This is noted in the graph below with an asterisk (\*), as this may have an unknown performance impact.
