...
We test parsing against both valid and invalid data. We also track whether the library returns the first issue found, or all of them (or both if configurable). Benchmarks that abort early are noted in the graph below with a dagger (†).
