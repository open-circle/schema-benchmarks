...
function FromRef$6(context, ref, result) {
	return result.includes(ref) ? result : ref in context ? FromType$13(context, context[ref], [...result, ref]) : Unreachable();
}
...
function FromType$12(type) {
	return IsRef$1(type) ? FromRef$5(type.$ref) : IsArray(type) ? Array$1(FromType$12(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$12(type.iteratorItems)) : IsConstructor(type) ? Constructor(FromTypes(type.parameters), FromType$12(type.instanceType)) : IsFunction(type) ? _Function_(FromTypes(type.parameters), FromType$12(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes(type.allOf)) : IsIterator(type) ? Iterator(FromType$12(type.iteratorItems)) : IsObject(type) ? _Object_(FromProperties$1(type.properties)) : IsPromise(type) ? Promise$1(FromType$12(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$12(RecordValue(type))) : IsUnion(type) ? Union(FromTypes(type.anyOf)) : IsTuple(type) ? Tuple(FromTypes(type.items)) : type;
}
