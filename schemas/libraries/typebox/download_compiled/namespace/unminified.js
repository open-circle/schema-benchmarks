...
function FromRef$6(context, ref, result) {
	return result.includes(ref) ? result : ref in context ? FromType$17(context, context[ref], [...result, ref]) : Unreachable();
}
...
function FromType$16(type) {
	return IsRef$1(type) ? FromRef$5(type.$ref) : IsArray(type) ? Array$1(FromType$16(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$16(type.iteratorItems)) : IsConstructor(type) ? Constructor(FromTypes(type.parameters), FromType$16(type.instanceType)) : IsFunction(type) ? _Function_(FromTypes(type.parameters), FromType$16(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes(type.allOf)) : IsIterator(type) ? Iterator(FromType$16(type.iteratorItems)) : IsObject(type) ? _Object_(FromProperties$1(type.properties)) : IsPromise(type) ? _Promise_(FromType$16(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$16(RecordValue(type))) : IsUnion(type) ? Union(FromTypes(type.anyOf)) : IsTuple(type) ? Tuple(FromTypes(type.items)) : type;
}
...
function FromTuple$7(types) {
	return FromType$13(TupleToObject(Tuple(types)));
}
//#endregion
...
function FromUnion$9(types) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? ReduceVariants(right, FromType$13(left)) : Unreachable();
}
//#endregion
...
function CollapseToObject(type) {
	return _Object_(FromType$13(type));
}
//#endregion
