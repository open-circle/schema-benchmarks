...
function FromType$18(type) {
	return IsRef$1(type) ? FromRef$7(type.$ref) : IsArray(type) ? Array$1(FromType$18(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$18(type.iteratorItems)) : IsConstructor(type) ? Constructor(FromTypes(type.parameters), FromType$18(type.instanceType)) : IsFunction(type) ? _Function_(FromTypes(type.parameters), FromType$18(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes(type.allOf)) : IsIterator(type) ? Iterator(FromType$18(type.iteratorItems)) : IsObject(type) ? _Object_(FromProperties$1(type.properties)) : IsPromise(type) ? _Promise_(FromType$18(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$18(RecordValue(type))) : IsUnion(type) ? Union(FromTypes(type.anyOf)) : IsTuple(type) ? Tuple(FromTypes(type.items)) : type;
}
...
function FromUnion$11(types) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? ReduceVariants(right, FromType$15(left)) : Unreachable();
}
//#endregion
