...
function FromTypesReduce(types) {
	...
	return IsSchema$1(left) ? FromType$20(left) ? FromTypesReduce(right) : false : true;
}
...
function FromUnion$11(variants, types, result = []) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? FromUnion$11(variants, right, [...result, ...FromType$19(variants, left)]) : result;
}
function FromType$19(variants, type) {
...
function DecodeFromSpan(variants, types) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? DecodeFromSpan(FromType$19(variants, left), right) : variants;
}
...
function FromRef$7(stack, context, ref) {
	return stack.includes(ref) ? true : FromType$18([...stack, ref], context, context[ref]);
}
...
function FromTypes$2(stack, context, types) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? FromType$18(stack, context, left) ? true : FromTypes$2(stack, context, right) : false;
}
function FromType$18(stack, context, type) {
	return IsRef$1(type) ? FromRef$7(stack, context, type.$ref) : IsArray(type) ? FromType$18(stack, context, type.items) : IsAsyncIterator(type) ? FromType$18(stack, context, type.iteratorItems) : IsConstructor(type) ? FromTypes$2(stack, context, [...type.parameters, type.instanceType]) : IsFunction(type) ? FromTypes$2(stack, context, [...type.parameters, type.returnType]) : IsInterfaceDeferred(type) ? FromProperties$3(stack, context, type.parameters[1]) : IsIntersect(type) ? FromTypes$2(stack, context, type.allOf) : IsIterator(type) ? FromType$18(stack, context, type.iteratorItems) : IsObject(type) ? FromProperties$3(stack, context, type.properties) : IsPromise(type) ? FromType$18(stack, context, type.item) : IsUnion(type) ? FromTypes$2(stack, context, type.anyOf) : IsTuple(type) ? FromTypes$2(stack, context, type.items) : IsRecord(type) ? FromType$18(stack, context, RecordValue(type)) : false;
}
/** Performs a cyclic check on the given type. Initial key stack can be empty, but faster if specified */
function CyclicCheck(stack, context, type) {
	return FromType$18(stack, context, type);
}
//#endregion
...
function FromRef$6(context, ref, result) {
	return result.includes(ref) ? result : ref in context ? FromType$17(context, context[ref], [...result, ref]) : Unreachable$1();
}
...
function FromTypes$1(context, types, result) {
	return types.reduce((result, left) => {
		return FromType$17(context, left, result);
	}, result);
}
function FromType$17(context, type, result) {
	return IsRef$1(type) ? FromRef$6(context, type.$ref, result) : IsArray(type) ? FromType$17(context, type.items, result) : IsAsyncIterator(type) ? FromType$17(context, type.iteratorItems, result) : IsConstructor(type) ? FromTypes$1(context, [...type.parameters, type.instanceType], result) : IsFunction(type) ? FromTypes$1(context, [...type.parameters, type.returnType], result) : IsInterfaceDeferred(type) ? FromProperties$2(context, type.parameters[1], result) : IsIntersect(type) ? FromTypes$1(context, type.allOf, result) : IsIterator(type) ? FromType$17(context, type.iteratorItems, result) : IsObject(type) ? FromProperties$2(context, type.properties, result) : IsPromise(type) ? FromType$17(context, type.item, result) : IsUnion(type) ? FromTypes$1(context, type.anyOf, result) : IsTuple(type) ? FromTypes$1(context, type.items, result) : IsRecord(type) ? FromType$17(context, RecordValue(type), result) : result;
}
...
function CyclicDependencies(context, key, type) {
	return FromType$17(context, type, [key]);
}
//#endregion
...
function FromProperties$1(properties) {
	return Keys(properties).reduce((result, key) => {
		return {
			...result,
			[key]: FromType$16(properties[key])
		};
	}, {});
}
...
function FromType$16(type) {
	return IsRef$1(type) ? FromRef$5(type.$ref) : IsArray(type) ? Array$1(FromType$16(type.items), ArrayOptions(type)) : IsAsyncIterator(type) ? AsyncIterator(FromType$16(type.iteratorItems)) : IsConstructor(type) ? Constructor$1(FromTypes(type.parameters), FromType$16(type.instanceType)) : IsFunction(type) ? _Function_$1(FromTypes(type.parameters), FromType$16(type.returnType)) : IsIntersect(type) ? Intersect(FromTypes(type.allOf)) : IsIterator(type) ? Iterator(FromType$16(type.iteratorItems)) : IsObject(type) ? _Object_$1(FromProperties$1(type.properties)) : IsPromise(type) ? _Promise_(FromType$16(type.item)) : IsRecord(type) ? Record(RecordKey(type), FromType$16(RecordValue(type))) : IsUnion(type) ? Union(FromTypes(type.anyOf)) : IsTuple(type) ? Tuple$1(FromTypes(type.items)) : type;
}
function CyclicAnyFromParameters(defs, ref) {
	return ref in defs ? FromType$16(defs[ref]) : Unknown();
}
...
function FromRef$4(context, ref, arguments_) {
	return ref in context ? FromType$15(context, ref, context[ref], arguments_) : FromNotResolvable();
}
function FromType$15(context, name, target, arguments_) {
...
function ResolveTarget(context, target, arguments_) {
	return FromType$15(context, "(anonymous)", target, arguments_);
}
//#endregion
...
function FromTemplateLiteral$3(mapping, pattern) {
	return FromType$14(mapping, TemplateLiteralDecode(pattern));
}
//#endregion
...
function CapitalizeAction(type, options) {
	return CanInstantiate([type]) ? Update$1(FromType$14(CapitalizeMapping, type), {}, options) : CapitalizeDeferred(type, options);
}
function LowercaseAction(type, options) {
	return CanInstantiate([type]) ? Update$1(FromType$14(LowercaseMapping, type), {}, options) : LowercaseDeferred(type, options);
}
function UncapitalizeAction(type, options) {
	return CanInstantiate([type]) ? Update$1(FromType$14(UncapitalizeMapping, type), {}, options) : UncapitalizeDeferred(type, options);
}
function UppercaseAction(type, options) {
	return CanInstantiate([type]) ? Update$1(FromType$14(UppercaseMapping, type), {}, options) : UppercaseDeferred(type, options);
}
...
function FromIntersect$8(types) {
	return types.reduce((result, left) => {
		return CollapseIntersectProperties(result, FromType$13(left));
	}, {});
}
//#endregion
...
function FromTuple$7(types) {
	return FromType$13(TupleToObject(Tuple$1(types)));
}
//#endregion
...
function ReduceVariants(types, result) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? ReduceVariants(right, CollapseUnionProperties(result, FromType$13(left))) : result;
}
function FromUnion$9(types) {
	const [left, ...right] = types;
	return IsSchema$1(left) ? ReduceVariants(right, FromType$13(left)) : Unreachable$1();
}
//#endregion
...
function CollapseToObject(type) {
	return _Object_$1(FromType$13(type));
}
//#endregion
...
function FromCyclic$7(defs, ref) {
	return FromType$12(CyclicTarget(defs, ref));
}
//#endregion
...
function FromIntersect$7(types) {
	return FromType$12(EvaluateIntersect(types));
}
//#endregion
...
function FromTemplateLiteral$2(pattern) {
	return FromType$12(TemplateLiteralDecode(pattern));
}
//#endregion
