...
function LegacyRetrievedResource(root, lexicalSchema, referenceBase, ref, schema) {
	...
	if (IsUndefined(targetBase) || IsEqual(targetBase, referenceBase)) return void 0;
	return {
		target: schema,
		base: targetBase,
		root: lexicalSchema
...
function FindRetrievedResource(stackframe, ref, schema, canonical, isRemote) {
	...
	if (!IsUndefined(remote)) return remote;
...
_Stack_instances = /* @__PURE__ */ new WeakSet(), _Stack_StackFrame = function _Stack_StackFrame() {
	...
	if (!IsUndefined(result.schema)) this.pendingResource = true;
	if (!IsUndefined(result.resolvedResource)) __classPrivateFieldGet(this, _Stack_instances, "m", _Stack_EnterResolvedResource).call(this, result.resolvedResource.target, result.resolvedResource.resource);
	if (!IsUndefined(result.retrievedResource)) this.retrievedResources.set(result.retrievedResource.target, {
	...
	if (IsArray(schema)) return __classPrivateFieldGet(this, _Stack_instances, "m", _Stack_RegisterResourceAnchorArray).call(this, schema);
	if (!IsSchemaObject(schema)) return;
	...
	if (IsSchemaBoolean(schema)) return;
	if (IsArray(schema)) return __classPrivateFieldGet(this, _Stack_instances, "m", _Stack_UnregisterResourceAnchorArray).call(this, schema);
	if (!IsSchemaObject(schema)) return;
	const current = schema;
	if (!isRoot && IsId(current)) return;
