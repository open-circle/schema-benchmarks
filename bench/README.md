...
Create a new file in `bench/schemas` named `<library>.ts` with the following content:
...
Import the `getSchema` function in `bench/initialization.bench.ts` and add the following code:
